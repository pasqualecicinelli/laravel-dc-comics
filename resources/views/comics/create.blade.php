...
@section('main-content')
    <section class="container mt-5">
        ...
        <h1 class="my-2"> Aggiungi i Fumetti</h1>
        ...
        <form action="{{ route('comics.store') }}" method="POST">
            ...
            <input class="form-control @error('title') is-invalid @enderror mt-2" type="text" id="title" name="title"
                placeholder="Titolo" aria-label="default input example" value={{ old('title') }}>
            @error('title')
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control @error('thumb') is-invalid @enderror" id="thumb"
                    name="thumb" aria-describedby="basic-addon3 basic-addon4" value={{ old('thumb') }}>
                @error('thumb')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <input class="form-control mt-2 @error('series') is-invalid @enderror" type="text" id="series"
                name="series" placeholder="Serie" aria-label="default input example" value={{ old('series') }}>
            @error('series')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            ...
            <input class="form-control mt-2 @error('type') is-invalid @enderror" type="text" id="type"
                name="type" placeholder="Genere" aria-label="default input example" value={{ old('type') }}>
            @error('type')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            ...
            <textarea class="form-control mt-2 @error('description') is-invalid @enderror" type="text" id="description"
                name="description" placeholder="Descrizione" aria-label="default input example"> {{ old('description') }}</textarea>
            @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
